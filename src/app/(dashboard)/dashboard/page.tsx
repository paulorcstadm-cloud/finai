...
import { ACCOUNTS, TRANSACTIONS, GOALS, ALERTS, MONTHLY_SPEND, CATEGORY_SPEND } from '@/lib/mock-data'
...
export default function DashboardPage() {
  ...
  const totalBalance = isGabriel ? 5047.80 : ACCOUNTS.reduce((s, a) => s + a.balance, 0)
  const monthExpenses = isGabriel ? 1246.20 : 3847.50
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="rounded-2xl bg-gradient-to-br from-primary-500/20 via-primary-600/10 to-transparent border border-primary-500/20 p-4 sm:p-5 shadow-card">
          ...
          <p className="text-xs text-slate-600 mt-1">{isGabriel ? '2 contas' : '4 contas'}</p>
        </div>
        ...
        <div className="rounded-2xl bg-[#16161E] border border-white/[0.06] p-4 sm:p-5 shadow-card">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-emerald-400 font-medium">{savingsRate}%</span>
          </div>
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium">Economia</p>
          <p className="text-xl sm:text-2xl font-bold text-white mt-1 tabular-nums financial-number">
            {formatCurrency(savings)}
          </p>
          <p className="text-xs text-emerald-400 mt-1">↑ Acima da meta de 20%</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-2xl bg-[#16161E] border border-white/[0.06] p-5 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={MONTHLY_SPEND} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
              <defs>
        ...
        <div className="rounded-2xl bg-[#16161E] border border-white/[0.06] p-5 shadow-card">
          ...
          <p className="text-xs text-slate-500 mb-4">Maio 2026</p>
          <ResponsiveContainer width="100%" height={130}>
            <PieChart>
              <Pie data={CATEGORY_SPEND.slice(0, 6)} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" strokeWidth={0}>
                {CATEGORY_SPEND.slice(0, 6).map((e, i) => <Cell key={i} fill={e.color} />)}
              </Pie>
              <Tooltip content={<PieTip />} />
            </PieChart>
          </ResponsiveContainer>
          <div className="space-y-2 mt-3">
            {CATEGORY_SPEND.slice(0, 5).map(cat => (
