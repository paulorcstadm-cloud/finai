...
export default function AssinaturasPage() {
  ...
  const emptyForm = { name: '', amount: '', billingCycle: 'monthly' as typeof CYCLES[number], nextBilling: new Date().toISOString().slice(0,10), category: 'Entretenimento', icon: '📱', color: '#6366f1' }
  ...
  const renewingSoon = active.filter(s => { const d = daysUntil(s.nextBilling); return d >= 0 && d <= 7 })
  const renewingSoonTotal = renewingSoon.reduce((s, sub) => s + sub.amount, 0)
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl bg-amber-500/8 border border-amber-500/20 p-5">
          <div className="flex items-center gap-2 mb-2"><AlertCircle className="w-3.5 h-3.5 text-amber-400" /><p className="text-xs text-slate-500 uppercase tracking-wider">Vencem em 7d</p></div>
          <p className="text-2xl font-bold text-amber-400">{renewingSoon.length}</p>
          <p className="text-xs text-slate-500 mt-1">{formatCurrency(renewingSoonTotal)}</p>
        </div>
      </div>
        ...
        <div className="rounded-2xl bg-[#16161E] border border-primary-500/20 p-5 shadow-glow-sm animate-slide-up">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <div><label className="text-xs text-slate-500 mb-1.5 block">Ciclo</label>
              <select value={form.billingCycle} onChange={e => setForm(f => ({ ...f, billingCycle: e.target.value as typeof CYCLES[number] }))} className="finai-input w-full px-3 py-2.5 text-sm text-white">
              ...
              <div key={sub.id} className={cn('flex items-center gap-4 p-4 rounded-2xl border transition-all group', sub.isActive ? 'bg-[#16161E] border-white/[0.06] hover:border-white/[0.12]' : 'bg-white/[0.02] border-white/[0.04] opacity-60', isDel && 'border-rose-500/30')}>
                ...
                {isEdit ? (
                  <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    <select value={editForm.billingCycle || 'monthly'} onChange={e => setEditForm(f => ({ ...f, billingCycle: e.target.value as typeof CYCLES[number] }))} className="finai-input px-2 py-2 text-xs text-white"><option value="monthly">Mensal</option><option value="yearly">Anual</option></select>
